...
Harness vendors this skill from OpenClaw commit
`a9ac13b2efd7ee7d4d1df5759c4c9ec15bd8e8f2`. The Harness runtime, not a Git
hook or pull request, chooses review targets.

- Review only Harness-recorded commits with `--mode commit --commit <sha>`.
- For remediation verification, review the cumulative task diff with
  `--mode branch --base <original-parent-sha>` from the task worktree.
- Always write validated JSON to a temporary path outside the reviewed repository.
- A valid report with findings is a completed review, even though the helper exits
  non-zero. Missing/invalid JSON or an engine failure is an infrastructure failure.
- Never discover a PR, call `gh`, fetch, push, install a Git hook, or mutate the
  reviewed repository from the Harness review worker.
- Findings are routed back to the originating Harness task and implementation run;
