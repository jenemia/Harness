...
Initial implementation: after each successful run, the PM runtime inspects the latest completion output and changed files before choosing automatic follow-up creation, the role-to-role handoff, or Done transition. It records a `pm.evaluated` timeline event with output excerpt, changed files, detected follow-up/risk/verification signals, and includes the evaluation summary in automatic handoff metadata.
...
Initial implementation: PM planning and document planning expose web preview actions, preview endpoints, and CLI commands that return the same task breakdown, dependency indexes, initial statuses, and large-plan warnings without creating tasks. Plans at or above the project's configurable large-plan threshold require preview confirmation through the web UI or an explicit `allowLargePlan` flag in API/CLI calls. A later approval gate can reuse this preview before allowing very large task creation.
...
Initial implementation: the scheduler can start all Selected tasks while respecting each agent's `maxParallel` limit. PM planning can optionally auto-start Selected tasks after creating the plan and reports started/skipped counts in the UI. In automatic planning mode, the deterministic PM provider records the requested mode and the effective mode it chose, using workflow templates and sequential/dependency signals for chains while keeping independent bullet lists parallel. Dependent tasks are unblocked automatically when prerequisites complete through agent execution, when a human manually marks the prerequisite Done, or when a dependency is explicitly waived. The PM runtime records completion-output evaluations before configured or dynamic handoff decisions, and can create automatic follow-up backlog tasks from completion signals. Backlog stays a planning queue until work is promoted to Selected. Tasks can also be paused and resumed through the UI, API, and CLI; paused tasks stay out of scheduler runs until resumed to Selected. Tasks can be moved up or down within their current board column, and the scheduler uses Selected board order when choosing work. Parent tasks can be decomposed into parallel or sequential child tasks so large work can become a visible dependency graph without leaving the task drawer.
...
- Planning provider: deterministic local planning now, future LLM-backed PM planning later.
...
Initial implementation: provider command defaults can be configured globally and per project. Agent-specific CLI commands override project provider commands, and project provider commands inherit from global defaults. Provider command lookup supports OS/model-specific keys with fallback order `<platformProviderId>.<modelBackend>`, `<nodePlatform>.<modelBackend>`, then `<modelBackend>` so Mac, Windows, Linux, and model-specific CLI wrappers can diverge without changing runtime orchestration. Health reporting also surfaces scheduler readiness gaps when Selected tasks cannot start because dependencies, project concurrency, or agent concurrency prevent execution.

Initial implementation: runtime platform behavior is selected through explicit Node platform providers such as `node-darwin`, `node-win32`, and `node-linux`-style fallbacks. Runtime workspace behavior is selected through a workspace provider that owns task worktree/workspace creation, snapshotting, changed-file collection, commits, and merge operations. The first provider supports both Git worktrees and Harness-managed non-Git workspaces, selected by each task's `workspaceMode` or inferred automatically at task creation. PM planning is selected through a deterministic local planning provider that owns workflow-template planning, explicit-list planning, sequential/parallel dependency shaping, and large-plan warnings so a later LLM planner can be added behind the same interface. LLM providers receive mode-aware workspace context through `HARNESS_WORKSPACE_KIND`, `HARNESS_WORKSPACE_MODE`, `HARNESS_WORKSPACE_PATH`, and a compatibility `HARNESS_WORKTREE_PATH`. Runtime approval behavior is selected through a `local-human` approval provider that owns command approval policy, decision messages, and rejection reasons while the project database stores the approval records. Runtime policy behavior is selected through a `local-agent-policy` provider that blocks command-backed LLM providers unless the assigned agent allows `shell`, `llm-cli`, the provider kind, or the provider id, and forces approval for risky shell command patterns even when project command approvals are disabled. The provider catalog exposes the active platform provider label, OS id, shell, process group support, workspace provider capabilities, planning provider capabilities, approval provider capabilities, policy provider capabilities, LLM provider definitions, and OS/model-specific provider command key precedence through the API, UI Settings panel, and headless CLI. The Settings panel can insert the selected model's platform-specific or model-wide provider command key into global or project command JSON without overwriting existing entries.
...
Initial implementation: Harness stores global settings for app-wide defaults, global memory for cross-project user preferences, and project-local settings for default LLM backend, provider command defaults, default agent concurrency, project-wide concurrency, PM plan auto-start behavior, large-plan confirmation threshold, and command approval policy. Harness also stores project-local memory entries for conventions and preferences that should travel with agent execution.
