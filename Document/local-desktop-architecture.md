...
승인된 process preview는 detached Harness host process group 안에서 실행한다. host marker에는 executable, args, workspace cwd, log/exit marker와 owner instance만 기록하고 environment 값은 저장하지 않는다. host는 선택된 environment 값과 credential pattern을 stdout/stderr에서 제거한 뒤 64 KiB bounded log를 쓰며, application service는 readiness probe와 exit marker를 통해 `booting`, `live`, `crashed`, `stopped`를 project DB에 반영한다.

stop과 restart는 DB의 PID만 신뢰하지 않고 실제 process command line이 Harness host executable과 해당 preview marker path를 포함하는지 확인한 뒤 process group을 종료한다. desktop과 선택적 HTTP server는 시작 전에 preview recovery를 완료한다. 이전 owner가 남긴 검증된 orphan은 종료하지만 identity가 다른 PID는 외부 process로 간주해 종료하지 않고 stale preview record만 crashed로 전환한다.

task 카드는 연결된 preview 수와 집계 상태를 표시하고, 상세 화면은 runtime, PID, readiness URL, artifact path, approval, 오류와 bounded log tail을 표시한다. 시작·중지·재시작과 open action은 renderer에서 OS API를 직접 호출하지 않고 typed IPC 또는 선택적 HTTP transport를 통해 동일 application service로 전달한다. open action은 등록된 HTTPS/loopback URL이나 task workspace 내부에 현재 존재하는 artifact만 OS의 argument-safe opener로 전달하며, 실행 직전 realpath 경계를 다시 검증한다.
