...
3. 결정 toast는 자동으로 사라지지 않고 질문 요약과 `카드 열기` action을 표시한다.
4. 사용자가 toast를 선택하면 Harness main window가 해당 card와 interaction을 연다.

### 완료와 실패

- 완료되면 짧은 `celebrating` animation과 완료 toast를 표시한다.
- 실패하면 `error` animation과 재시도 또는 카드 열기 action을 표시한다.
...
캐릭터 hover 또는 keyboard focus 시 다음 정보를 표시한다.
...
- 완료 toast는 8초 후 사라진다.
- question, approval, permission과 error toast는 사용자가 확인할 때까지 유지한다.
- toast 선택 시 main Harness window를 활성화하고 해당 card 또는 interaction을 연다.
...
- 기본적으로 click-through로 동작해 뒤 application 사용을 방해하지 않는다.
- actionable toast와 tooltip 영역에 pointer가 들어왔을 때만 제한된 interaction mode로 전환한다.
...
- click-through와 actionable region 전환
...
- toast click은 임의 URL이 아니라 typed `open-task` IPC만 호출한다.

## 접근성

- reduced motion에서는 정적 sprite와 상태 색상·text만 표시한다.
- animation 완전 정지 option을 제공한다.
- tooltip과 toast에 keyboard focus와 screen reader label을 제공한다.
...
- transparent always-on-top window와 click-through를 구현한다.
...
- 중요 event 기반 toast queue, dedupe, sticky decision과 card open action을 구현한다.
- main window focus와 card deep-link를 연결한다.
...
- Windows 11 multi-monitor, DPI, virtual desktop, full-screen과 click-through를 검증한다.
...
- toast stack overflow와 sticky interaction
- reduced motion과 scale 변경

### Desktop integration

- provider event → activity state → overlay IPC → sprite state
- toast click → main window 활성화 → task card open
