...
- [x] Electron main process가 SQLite, Git, provider process, scheduler와 policy를 담당하고 React renderer는 typed IPC만 사용한다.
- [x] production desktop은 packaged web asset을 열고 별도의 persistent HTTP server를 자동 시작하지 않는다.
- [x] HTTP transport는 headless 또는 remote 사용을 위한 명시적 선택 기능으로만 남긴다.
...
- [x] 업무 카드 작성 팝업을 `초안 편집 영역`과 `실시간 코멘트 영역`의 2열 레이아웃으로 구성한다.
- [x] 작은 화면에서는 코멘트 영역을 탭 또는 drawer로 전환한다.
- [x] 초안 수정 시 revision을 증가시키고 debounce 후 검토 요청을 전송한다.
- [x] 코멘트에 작성 에이전트, 대상 draft revision, 생성 시각, 상태를 표시한다.
- [x] 코멘트 상태로 `검토 중`, `제안`, `질문`, `위험`, `해결됨`, `반영됨`을 지원한다.
- [x] 사용자가 코멘트 스레드에서 답변하고 에이전트를 직접 언급할 수 있게 한다.
- [x] 에이전트가 응답 중일 때 스트리밍 상태와 중지·재시도 동작을 제공한다.
- [x] `내용 반영` 버튼은 반영할 코멘트를 선택할 수 있게 하고 기획 에이전트를 호출한다.
- [x] 기획 에이전트의 결과를 원문과 diff로 비교한 뒤 승인하도록 한다.
- [x] 승인 전에는 에이전트가 사용자 초안을 직접 변경하지 못하게 한다.
- [x] 반영 직후 실행 취소와 이전 revision 복원을 지원한다.
- [x] 팝업을 닫았다 다시 열어도 초안과 코멘트가 복구되게 한다.

### 서버 및 데이터 TODO

- [x] 저장되지 않은 카드 초안을 나타내는 draft 세션 모델을 추가한다.
- [x] draft revision, 참여 에이전트, 코멘트, 사용자 답변과 반영 이력을 영속화한다.
- [x] 오래된 revision을 검토한 에이전트 응답은 표시하되 자동 반영 대상에서 제외한다.
- [x] 검토 요청의 debounce, 취소, 중복 제거와 에이전트별 rate limit을 구현한다.
- [x] 실시간 응답 전송 방식을 정하고 연결 재수립 시 누락 이벤트를 복구한다.
- [x] 에이전트 간 코멘트가 무한히 서로를 호출하지 않도록 사용자 입력과 초안 변경만 자동 검토 trigger로 삼는다.
- [x] `내용 반영` 요청에 draft revision과 선택한 comment id를 포함해 중복 실행을 방지한다.
- [x] 기획 에이전트 결과에 정리된 본문, 완료 조건, 의존성, 위험, 미결 질문, 변경 요약을 구조화해 반환한다.
...
- [x] 사용자가 코멘트 또는 승인 UI로 응답하면 동일 run의 맥락을 복구해 재개한다.
- [x] 중복 응답, 만료된 요청, 취소된 run과 서버 재시작을 안전하게 처리한다.
- [x] 대기 중인 interaction을 Attention 패널, 카드, CLI에서 동일하게 조회할 수 있게 한다.
- [x] 질문 응답, 승인과 거절, 재개 결과를 task timeline에 기록한다.
